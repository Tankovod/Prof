...
from src.dependencies import is_user_authorized, user_auth
...
async def get_product(request: Request, user_or_401: Union[UserView, status] = user_auth):
...
async def get_user_profile(request: Request, user: UserView = user_auth):
...
async def get_home(request: Request, user_or_401: Union[UserView, status] = is_user_authorized):
    async with Product.async_session() as session:
        products = await session.scalars(
            select(Product).options(selectinload(Product.images), selectinload(Product.units)))
